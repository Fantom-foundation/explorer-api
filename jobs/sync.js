...
**/
const writeBlockToDB = function (config, blockData, flush) {
  const self = writeBlockToDB;
  if (!self.bulkOps) {
    self.bulkOps = [];
  }
  if (blockData && blockData.number >= 0) {
    const block = { ...blockData }; // break object reference
...
var syncChain = function (config, nextBlock) {
  try {
    ...
    setTimeout(() => { console.log(`syncChain, next block: ${nextBlock}`); syncChain(config, nextBlock); }, 500);
  } catch (err) {
    console.log(err);
